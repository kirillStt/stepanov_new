...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
            ...
            showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    const Text('Пожалуйста, подтвердите ваш email, перейдя по ссылке в письме.'),
                    const SizedBox(height: 16),
                    Text(_emailController.text.trim(), style: const TextStyle(fontWeight: FontWeight.bold)),
                  ],
                ),
                actions: [
          ...
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  const Text('На указанный адрес отправлено письмо с кодом подтверждения.', style: TextStyle(fontSize: 16)),
                  const SizedBox(height: 16),
                  Text(_emailController.text.trim(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  const SizedBox(height: 16),
                  const Text('Введите код из письма на следующем экране.', style: TextStyle(fontSize: 14, color: Colors.grey)),
                ],
              ),
              actions: [
                TextButton(
      ...
      else if (e.message.contains('Email not confirmed')) message = 'Email не подтверждён. Проверьте почту';
      else if (e.message.contains('User already registered')) message = 'Пользователь с таким email уже зарегистрирован';
      else if (e.message.contains('Password should be at least 6 characters')) message = 'Пароль должен быть не менее 6 символов';
      else if (e.message.contains('rate limit')) message = 'Слишком много попыток. Попробуйте позже';
      else message = 'Ошибка: ${e.message}';
    ...
    try {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('На адрес $email отправлен код для сброса пароля. Введите этот код на следующем экране.'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Container(
          constraints: const BoxConstraints(maxWidth: 400, minWidth: 380, maxHeight: 500, minHeight: 480),
          child: Padding(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Подтвердите ваш email', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Text('Мы отправили письмо на адрес:', style: TextStyle(fontSize: 16, color: Colors.grey[600])),
...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  Future<void> _verifyAndReset() async {
    ...
    if (token.isEmpty) { _showMessage('Введите код из письма', isError: true); return; }
    if (newPassword.isEmpty || confirmPassword.isEmpty) { _showMessage('Заполните все поля', isError: true); return; }
    if (newPassword != confirmPassword) { _showMessage('Пароли не совпадают', isError: true); return; }
    if (newPassword.length < 6) { _showMessage('Пароль должен быть не менее 6 символов', isError: true); return; }

    setState(() => _isLoading = true);
    try {
      final response = await supabase.auth.verifyOTP(type: OtpType.recovery, token: token, email: email);
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Text('Введите код из письма и новый пароль', style: TextStyle(fontSize: 16), textAlign: TextAlign.center),
            const SizedBox(height: 24),
            TextField(controller: _tokenController, decoration: const InputDecoration(labelText: 'Код из письма', prefixIcon: Icon(Icons.vpn_key), border: OutlineInputBorder())),
            const SizedBox(height: 16),
            TextField(controller: _newPasswordController, obscureText: true, decoration: const InputDecoration(labelText: 'Новый пароль', prefixIcon: Icon(Icons.lock), border: OutlineInputBorder())),
            const SizedBox(height: 16),
            TextField(controller: _confirmPasswordController, obscureText: true, decoration: const InputDecoration(labelText: 'Подтвердите пароль', prefixIcon: Icon(Icons.lock_outline), border: OutlineInputBorder())),
            const SizedBox(height: 24),
...
class _FileManagerScreenState extends State<FileManagerScreen> with AutomaticKeepAliveClientMixin {
  ...
  void _applyFilters() {
    setState(() {
      filteredFiles = files.where((file) {
        if (searchQuery.isNotEmpty && !file.name.toLowerCase().contains(searchQuery.toLowerCase())) return false;
        if (sizeFilter != 'all') {
          final sizeMB = file.size / (1024 * 1024);
          switch (sizeFilter) {
            case 'small': if (sizeMB > 1) return false; break;
            case 'medium': if (sizeMB < 1 || sizeMB > 10) return false; break;
            case 'large': if (sizeMB < 10) return false; break;
          }
        }
        if (dateFilter != 'all') {
          final difference = DateTime.now().difference(file.modifiedAt).inDays;
          switch (dateFilter) {
            case 'today': if (difference > 1) return false; break;
            case 'week': if (difference > 7) return false; break;
            case 'month': if (difference > 30) return false; break;
          }
        }
        if (typeFilter != 'all') {
          final ext = file.name.split('.').last.toLowerCase();
          const imageTypes = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'];
          const docTypes = ['txt', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'];
          const archiveTypes = ['zip', 'rar', '7z', 'tar', 'gz'];
          switch (typeFilter) {
            case 'image': if (!imageTypes.contains(ext)) return false; break;
            case 'document': if (!docTypes.contains(ext)) return false; break;
            case 'archive': if (!archiveTypes.contains(ext)) return false; break;
            case 'other': if (imageTypes.contains(ext) || docTypes.contains(ext) || archiveTypes.contains(ext)) return false; break;
          }
        }
        return true;
      }).toList();
  ...
  Future<bool> _showOverwriteDialog(String fileName) async {
    return await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Файл "$fileName" уже загружен.\n\nХотите заменить его?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Отмена')),
          TextButton(onPressed: () => Navigator.pop(context, true), style: TextButton.styleFrom(foregroundColor: Colors.orange), child: const Text('Заменить')),
        ],
      ),
    ) ?? false;
  }
  ...
  Future<void> pickAndUploadFile() async {
    ...
    try {
      FilePickerResult? result = await FilePicker.platform.pickFiles(withData: false);
      if (result != null) {
        final platformFile = result.files.single;
        final fileName = platformFile.name;
        final filePath = platformFile.path;
        
        if (filePath == null) { _showMessage('Не удалось получить путь к файлу', isError: true); if (mounted) setState(() => isLoading = false); return; }
        
        final userId = _userId ?? '';
        if (userId.isEmpty) { _showMessage('Пользователь не авторизован', isError: true); if (mounted) setState(() => isLoading = false); return; }
        
        final fileExists = files.any((f) => f.name == fileName);
        if (fileExists) {
          if (!await _showOverwriteDialog(fileName)) {
            if (mounted) setState(() => isLoading = false);
            _showMessage('Загрузка отменена');
            return;
          }
        }
        ...
        final uploaded = await storageService.uploadFile(file, fileName, userId, mimeType: mimeType);
  ...
  Future<void> _downloadFile(FileModel file) async {
    try {
      String? downloadUrl = file.downloadUrl ?? await storageService.getDownloadUrl(file.path);
      if (downloadUrl == null || downloadUrl.isEmpty) {
        _showMessage('Не удалось получить ссылку для скачивания', isError: true);
        return;
      }
      
      String? outputPath = await FilePicker.platform.saveFile(dialogTitle: 'Сохранить файл', fileName: file.name);
      if (outputPath != null) {
        _showMessage('Скачивание началось...');
        final request = await HttpClient().getUrl(Uri.parse(downloadUrl));
        final response = await request.close();
        if (response.statusCode == 200) {
          await File(outputPath).writeAsBytes(await consolidateHttpClientResponseBytes(response));
          if (mounted) _showMessage('Файл сохранён');
        } else if (mounted) {
          _showMessage('Ошибка скачивания: ${response.statusCode}', isError: true);
        }
      }
    } catch (e) {
      if (mounted) _showMessage('Ошибка при сохранении: $e', isError: true);
    }
  }
  ...
  Future<void> _shareFile(FileModel file) async {
    try {
      String? downloadUrl = file.downloadUrl ?? await storageService.getDownloadUrl(file.path);
      if (downloadUrl != null) {
        final downloadLink = downloadUrl.contains('?') ? '$downloadUrl&download' : '$downloadUrl?download';
  ...
  Future<void> _deleteFile(int index) async {
    ...
    if (await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      ...
      if (await storageService.deleteFile(file.path)) {
        (await SharedPreferences.getInstance()).remove('file_name_${file.id}');
        setState(() {
          filteredFiles.removeAt(index);
          if (originalIndex != -1) files.removeAt(originalIndex);
        });
  ...
  void _navigateToChangePassword() => Navigator.push(context, MaterialPageRoute(builder: (context) => const ChangePasswordScreen()));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              if (value == 'refresh') _loadFiles();
              else if (value == 'filter') setState(() => isFilterVisible = !isFilterVisible);
              else if (value == 'theme') themeProvider.toggleTheme();
              else if (value == 'password') _navigateToChangePassword();
              else if (value == 'logout') _logout();
            },
            itemBuilder: (context) => [
              const PopupMenuItem(value: 'filter', child: Row(children: [Icon(Icons.filter_alt, color: Colors.blue, size: 20), SizedBox(width: 8), Text('Фильтры')])),
              const PopupMenuItem(value: 'refresh', child: Row(children: [Icon(Icons.refresh, color: Colors.green, size: 20), SizedBox(width: 8), Text('Обновить')])),
              const PopupMenuItem(value: 'theme', child: Row(children: [Icon(Icons.brightness_6, color: Colors.purple, size: 20), SizedBox(width: 8), Text('Тема')])),
              const PopupMenuItem(value: 'password', child: Row(children: [Icon(Icons.password, color: Colors.orange, size: 20), SizedBox(width: 8), Text('Сменить пароль')])),
              const PopupMenuItem(value: 'logout', child: Row(children: [Icon(Icons.logout, color: Colors.red, size: 20), SizedBox(width: 8), Text('Выйти', style: TextStyle(color: Colors.red))])),
            ],
          ),
        ],
      ),
          ...
          if (isFilterVisible)
            Container(
              ...
              child: Column(
                children: [
                  TextField(
                    decoration: const InputDecoration(labelText: 'Поиск по имени', prefixIcon: Icon(Icons.search), border: OutlineInputBorder()),
                  ...
                  Row(children: [
                    ...
                    Expanded(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) { setState(() { sizeFilter = value!; _applyFilters(); }); },
                      ),
                    ),
                  ]),
                  const SizedBox(height: 8),
                  Row(children: [
                    ...
                    Expanded(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) { setState(() { dateFilter = value!; _applyFilters(); }); },
                      ),
                    ),
                  ]),
                  const SizedBox(height: 8),
                  Row(children: [
                    ...
                    Expanded(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) { setState(() { typeFilter = value!; _applyFilters(); }); },
                      ),
                    ),
                  ]),
                  const SizedBox(height: 12),
                  TextButton.icon(onPressed: _resetFilters, icon: const Icon(Icons.clear), label: const Text('Сбросить фильтры')),
                ],
              ),
            ),
          Expanded(
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 20),
                            Text(files.isEmpty ? 'Нет файлов' : 'Нет файлов по выбранным фильтрам', style: const TextStyle(fontSize: 18, color: Colors.grey)),
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              onTap: _isImageFile(file.name)
                                  ? () async {
                                      String? imageUrl = file.downloadUrl ?? await storageService.getDownloadUrl(file.path);
                                      if (imageUrl != null && imageUrl.isNotEmpty && context.mounted) {
                                        Navigator.push(context, MaterialPageRoute(builder: (context) => ImagePreviewScreen(file: file, imageUrl: imageUrl)));
                                      } else {
                              ...
                              trailing: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  IconButton(icon: const Icon(Icons.download, color: Colors.green), onPressed: () => _downloadFile(file), tooltip: 'Скачать'),
                                  IconButton(icon: const Icon(Icons.share, color: Colors.blue), onPressed: () => _shareFile(file), tooltip: 'Поделиться'),
                                  IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteFile(index), tooltip: 'Удалить'),
                                ],
                              ),
                            ),
                          );
      ...
      floatingActionButton: FloatingActionButton(onPressed: pickAndUploadFile, tooltip: 'Загрузить файл', child: const Icon(Icons.add)),
    );
  }
